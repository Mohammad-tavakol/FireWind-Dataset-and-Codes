...
SR_estimated_directories=["/home/availab-dl2/Sandboxes/Mohammad/FireWind-Dataset/FireWind_SR_estimations/5by5",
              "/home/availab-dl2/Sandboxes/Mohammad/FireWind-Dataset/FireWind_SR_estimations/7by7",
              "/home/availab-dl2/Sandboxes/Mohammad/FireWind-Dataset/FireWind_SR_estimations/10by10",
              "/home/availab-dl2/Sandboxes/Mohammad/FireWind-Dataset/FireWind_SR_estimations/pb_25",
              "/home/availab-dl2/Sandboxes/Mohammad/FireWind-Dataset/FireWind_SR_estimations/pb_100",
              "/home/availab-dl2/Sandboxes/Mohammad/FireWind-Dataset/FireWind_SR_estimations/random_25",
              "/home/availab-dl2/Sandboxes/Mohammad/FireWind-Dataset/FireWind_SR_estimations/random_49",
              "/home/availab-dl2/Sandboxes/Mohammad/FireWind-Dataset/FireWind_SR_estimations/random_100",
              "/home/availab-dl2/Sandboxes/Mohammad/FireWind-Dataset/FireWind_highres" ]
...
def training_data_generator(batch_size=batch_size, n_target=n_input, high_altitude_directories=SR_estimated_directories, 
                            Target_directory=Target_directory,Input_directory=Input_directory):

    for high_altitude_directory in high_altitude_directories:
        print(f"processing {high_altitude_directory}")
        indices = np.arange(n_target)
        np.random.shuffle(indices)
        if high_altitude_directory == f"/home/availab-dl2/Sandboxes/Mohammad/FireWind-Dataset/FireWind_highres":
            ...
            for i in range(0, n_target, batch_size):
                ...
                reconstructed_batch = np.array([np.load(f"{high_altitude_directory}/X_train/%d.npy" % j) for j in batch_indices])
                ...
                terrain_batch = np.array([np.load(f"{Input_directory}/X_train/%d.npy" % j) for j in batch_indices])[:,4:8]   ##loading high res terrain., vegetation and fire data
...
def validation_data_generator(batch_size=batch_size, n_target=n_valid, high_altitude_directory=SR_estimated_directories[0], 
                            Target_directory=Target_directory,Input_directory=Input_directory):
    indices = np.arange(n_target)
    for i in range(0, n_target, batch_size):
        batch_indices = indices[i:i+batch_size]
        reconstructed_batch = np.array([np.load(f"{high_altitude_directory}/X_valid/%d.npy" % j) for j in batch_indices])
        reconstructed_batch=np.reshape(reconstructed_batch,[batch_size,4,416,416])  ##loading reconstructed data from SR step
        reconstructed_batch[:, 3] = reconstructed_batch[:, 3] / 4  # Normalising T40
        #print(reconstructed_batch.shape)

        terrain_batch = np.array([np.load(f"{Input_directory}/X_valid/%d.npy" % j) for j in batch_indices])[:,4:8]   ##loading high res terrain., vegetation and fire data
        terrain_batch[:, 0] = terrain_batch[:, 0] / 1500
        terrain_batch[:, 1] = terrain_batch[:, 1] / 90
        terrain_batch[:, 2] = terrain_batch[:, 2] / 2
        input_batch=np.concatenate((reconstructed_batch,terrain_batch),axis=1)
...
# # load model
model = load_model(f"/home/availab-dl2/Sandboxes/Mohammad/DownscalingModel.keras",custom_objects={"CBAM": CBAM}) 


cp = tf.keras.callbacks.ModelCheckpoint(f"/home/availab-dl2/Sandboxes/Mohammad/DownscalingModel-finetune-alldata.keras",
                                         monitor="val_loss", mode = 'auto', save_best_only=True )

history=model.fit(train_data, steps_per_epoch=len(SR_estimated_directories)*(n_input//batch_size), validation_data=valid_data, 
                  validation_steps=(n_valid//batch_size), epochs=30 ,callbacks=[ cp],verbose=2)
...
plt.legend()
plt.savefig(f'/home/availab-dl2/Sandboxes/Mohammad/DownscalingModel-finetune.png')
plt.show()